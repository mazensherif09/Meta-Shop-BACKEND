...
const handleMerageCartItems = (items1 = [], items2 = []) => {
  ...
  combinedItems.forEach((item) => {
    const duplicateItem = uniqueItems.find(
      (uniqueItem) =>
        uniqueItem?.product?._id?.toString() ===
    ...
    try {
      ...
      if (decoded?.cart) {
        ...
        if (localCart) {
          ...
          // console.log("🚀 ~ handleConnectCart ~ localItems, cart?.items:", localItems, cart?.items)
          cart.items = handleMerageCartItems(localCart.items, cart?.items);
